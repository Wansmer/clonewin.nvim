...
local default_config = {
  ...
  log_file = {
    enabled = true,
    path = vim.fs.joinpath(vim.fn.stdpath("cache"), "clonewin.log"),
  },
  stdout = {
    enabled = false,
    hl = {
...
function Logger._stringify(...)
  ...
  local function to_string(val)
    return vim.list_contains({ "string", "number", "boolean" }, type(val))
        and tostring(val)
  ...
  if type(args[1]) == "string" then
    ...
    while idx do
      ...
      message_to_format = message_to_format:sub(1, idx - 1)
        .. arg
        .. message_to_format:sub(idx + 2)
...
function Logger:log(level, ...)
  ...
  local msg =
    string.format("%s [%s] %s", timestamp, level, Logger._stringify(...))
  ...
  if self.config.stdout.enabled then
    vim.api.nvim_echo(
      { { msg, self.config.stdout.hl[level] or "None" } },
      true,
      {}
    )
  end
